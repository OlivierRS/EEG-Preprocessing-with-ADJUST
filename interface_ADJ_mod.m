...
function [EEG, art] = interface_ADJ_mod(EEG,report)
...
%         EEGep = pop_rmbase( EEGep, [0  0]);
   ...
   [art, horiz, vert, blink, disc,...
    ...
    fname=strcat(EEG.filepath, 'ADJ_report_', name, '.mat');

    save (fname, 'blink', 'horiz', 'vert', 'disc');

    disp(' ')
    disp(['Artifact ICs list saved in ' fname]);
...
%%
    ...
    if isempty(EEG.icaact)
        disp('Warning: EEG.icaact missing! Recomputed from EEG.icaweights, EEG.icasphere and EEG.data');
        % Next instruction: see eeg_checkset
        final_shape = [size(EEG.icaweights,1) size(EEG.data,2) size(EEG.data,3)];
        temp_shape = [size(EEG.icaweights,1) size(EEG.data,2)*size(EEG.data,3)];
        X = reshape(EEG.data(1:size(EEG.icaweights,1),:,:),temp_shape);
        EEG.icaact = reshape(EEG.icaweights*EEG.icasphere*X, final_shape);
    end
